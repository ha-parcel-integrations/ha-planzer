"""Sensor platform for the Planzer parcel tracker integration."""
...
from .coordinator import PlanzerCoordinator
...
async def async_setup_entry(
    hass: HomeAssistant,
    entry: PlanzerConfigEntry,
    async_add_entities: AddEntitiesCallback,
) -> None:
    """Set up Planzer sensor entities from a config entry."""
    ...
    entities: list[SensorEntity] = [
        PlanzerIncomingParcelsSensor(
            coordinator, entry, async_add_entities, current_barcodes
        ),
    ]
    for parcel in coordinator.data or []:
        entities.append(
            PlanzerParcelSensor(coordinator, entry, parcel.get("barcode", ""))
        )
    entities.append(PlanzerNextDeliverySensor(coordinator, entry))
    entities.append(PlanzerDeliveredParcelsSensor(coordinator, entry))
    entities.append(PlanzerLastUpdateSensor(coordinator, entry))

    async_add_entities(entities)


class PlanzerIncomingParcelsSensor(
    CoordinatorEntity[PlanzerCoordinator], SensorEntity
):
    ...
    def __init__(
        self,
        coordinator: PlanzerCoordinator,
        entry: ConfigEntry,
        async_add_entities: AddEntitiesCallback,
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if new_barcodes:
            self._async_add_entities(
                PlanzerParcelSensor(self.coordinator, self._entry, barcode)
...
class PlanzerParcelSensor(CoordinatorEntity[PlanzerCoordinator], SensorEntity):
    """Per-parcel sensor reporting the status of one tracked Planzer parcel."""
    ...
    def __init__(
        self, coordinator: PlanzerCoordinator, entry: ConfigEntry, barcode: str
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
        self._entry = entry
...
class PlanzerNextDeliverySensor(
    CoordinatorEntity[PlanzerCoordinator], SensorEntity
):
    ...
    def __init__(
        self, coordinator: PlanzerCoordinator, entry: ConfigEntry
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
...
class PlanzerDeliveredParcelsSensor(
    CoordinatorEntity[PlanzerCoordinator], SensorEntity
):
    """Recently delivered tracked Planzer parcels."""
...
class PlanzerLastUpdateSensor(
    CoordinatorEntity[PlanzerCoordinator], SensorEntity
):
    """Diagnostic sensor reporting when Planzer was last polled successfully."""
