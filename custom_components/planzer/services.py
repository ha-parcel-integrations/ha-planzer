"""Services for the Planzer parcel tracker integration.

`planzer.track_parcel` / `planzer.untrack_parcel` let you add or remove a
...
def _resolve_entry(hass: HomeAssistant):
    """Return the single Planzer hub, or raise when it is not set up."""
    entries = hass.config_entries.async_entries(DOMAIN)
    if not entries:
        raise ServiceValidationError("Planzer is not set up")
    return entries[0]


def async_setup_services(hass: HomeAssistant) -> None:
    """Register the Planzer services (idempotent)."""
    ...
    async def _track(call: ServiceCall) -> None:
        ...
        if not valid_tracking_code(tracking_code):
            raise ServiceValidationError(
                f"'{tracking_code}' is not a valid Planzer tracking code"
            )
...
def async_unload_services(hass: HomeAssistant) -> None:
    """Remove the Planzer services (single-entry integration, so on unload)."""
