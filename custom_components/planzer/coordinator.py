"""Coordinator for the Planzer parcel tracker integration.
...
from .api import PlanzerApiClient, PlanzerApiError
from .const import (
...
class PlanzerCoordinator(DataUpdateCoordinator[list[dict]]):
    ...
    async def _async_update_data(self) -> list[dict]:
        ...
        for code, result in zip(codes, results):
            if isinstance(result, BaseException):
                if not isinstance(
                    result, (PlanzerApiError, aiohttp.ClientError)
                ):
                    raise result
                errors += 1
                _LOGGER.warning("Planzer fetch failed for %s: %s", code, result)
            ...
            # The response's own shipment number can be missing on edge
        ...
        if codes and errors == len(codes) and not raws:
            raise UpdateFailed("Planzer unreachable for all tracked parcels")
