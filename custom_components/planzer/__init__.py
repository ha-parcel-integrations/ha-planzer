"""Planzer parcel tracker custom component for Home Assistant."""
...
from .api import PlanzerApiClient
from .const import PLATFORMS
from .coordinator import PlanzerCoordinator, _refresh_interval
...
@dataclass
class PlanzerData:
    """Runtime data attached to the Planzer config entry."""

    client: PlanzerApiClient
    coordinator: PlanzerCoordinator


type PlanzerConfigEntry = ConfigEntry[PlanzerData]


async def async_setup_entry(hass: HomeAssistant, entry: PlanzerConfigEntry) -> bool:
    """Set up Planzer from a config entry."""
    # No auth: Planzer tracking is public, so the HA-managed session is fine.
    client = PlanzerApiClient(async_get_clientsession(hass))
    coordinator = PlanzerCoordinator(hass, client, entry)
    ...
    entry.runtime_data = PlanzerData(client=client, coordinator=coordinator)
...
async def async_unload_entry(hass: HomeAssistant, entry: PlanzerConfigEntry) -> bool:
    """Unload the Planzer config entry."""
