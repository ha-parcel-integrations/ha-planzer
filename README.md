...
[![Downloads](https://img.shields.io/github/downloads/ha-parcel-integrations/ha-planzer/total.svg)](https://github.com/ha-parcel-integrations/ha-planzer/releases)
...
Instead of polling Planzer at the same rate around the clock, the
...
- **Fully stopped** — nothing is tracked, or every tracked parcel has been
  ...
  `planzer.track_parcel` service, or a dashboard button) resumes polling
  immediately.
- A small, fixed per-hub offset is added on top, so not every Planzer hub
...
This integration uses the same public tracking endpoint as the Planzer consumer tracking site. It is not affiliated with, endorsed by, or supported by Planzer AG.
