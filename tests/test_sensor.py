"""Tests for Planzer sensor property logic."""
from datetime import datetime, timezone
from unittest.mock import MagicMock

from custom_components.planzer.const import ParcelStatus
from custom_components.planzer.sensor import (
    PlanzerDeliveredParcelsSensor,
    PlanzerIncomingParcelsSensor,
    PlanzerLastUpdateSensor,
    PlanzerNextDeliverySensor,
    PlanzerParcelSensor,
)
...
def test_incoming_counts_and_lists():
    ...
    sensor = PlanzerIncomingParcelsSensor(coordinator, _entry(), lambda _: None, set())
...
def test_parcel_sensor_status_and_attributes():
    ...
    sensor = PlanzerParcelSensor(_coordinator([parcel]), _entry(), "A")
...
def test_parcel_sensor_missing_barcode():
    sensor = PlanzerParcelSensor(_coordinator([_parcel("A")]), _entry(), "OTHER")
...
def test_next_delivery_picks_earliest():
    ...
    sensor = PlanzerNextDeliverySensor(coordinator, _entry())
...
def test_next_delivery_none_without_moments():
    sensor = PlanzerNextDeliverySensor(_coordinator([_parcel("A")]), _entry())
    assert sensor.native_value is None
    assert sensor.extra_state_attributes == {}
...
def test_delivered_sensor():
    ...
    sensor = PlanzerDeliveredParcelsSensor(coordinator, _entry())
...
def test_last_update_sensor():
    ...
    sensor = PlanzerLastUpdateSensor(coordinator, _entry())
