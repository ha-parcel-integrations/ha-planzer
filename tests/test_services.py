"""Tests for the Planzer services (track_parcel / untrack_parcel)."""
...
async def test_track_parcel_adds_to_options(hass):
    ...
    with patch(
        ...
        await hass.services.async_call(
            DOMAIN,
            "track_parcel",
            {CONF_TRACKING_CODE: "12345679"},
            blocking=True,
        )
        await hass.async_block_till_done()

    parcels = entry.options[CONF_PARCELS]
    assert parcels == [{CONF_TRACKING_CODE: "12345679"}]
...
async def test_untrack_parcel_removes_from_options(hass):
    entry = await _setup(
        hass, parcels=[{CONF_TRACKING_CODE: "12345679"}]
    )
    with patch(
        "custom_components.planzer.api.PlanzerApiClient.async_get_parcel",
        new=AsyncMock(return_value=_SAMPLE),
