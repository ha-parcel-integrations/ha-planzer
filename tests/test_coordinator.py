"""Tests for the Planzer coordinator: fetching, caching and events.
...
from custom_components.planzer.api import PlanzerApiError
...
from custom_components.planzer.coordinator import PlanzerCoordinator
...
def _in_transit(code: str = ACTIVE_CODE) -> dict:
    """The same shipment, rewound to its "Übergeben" (handed over) scan."""
    sample = active_sample(code)
    sample["transportPositions"] = [position(EVENTS[:2])]
    return sample
...
async def test_update_keeps_cached_payload_on_error(hass):
    ...
    client.async_get_parcel.side_effect = PlanzerApiError("HTTP 500")
...
async def test_update_backfills_missing_tracking_number(hass):
    """An edge payload without a shipment number keeps the requested code."""
    entry = _entry_with([{CONF_TRACKING_CODE: OTHER_CODE}])
    entry.add_to_hass(hass)
...
async def test_fires_delivery_time_changed_event(hass):
    ...
    moved = active_sample()
    # Planzer reschedules by moving the delivery day, not by narrowing a window.
    moved["deliveryDay"]["date"] = "2026-03-18"
    client.async_get_parcel.return_value = moved
    await coordinator._async_update_data()
    await hass.async_block_till_done()

    assert len(events) == 1
    assert events[0].data["old_planned_from"] == "2026-03-16T00:00:00+01:00"
    assert events[0].data["new_planned_from"] == "2026-03-18T00:00:00+01:00"
...
async def test_losing_the_eta_is_silent(hass):
    ...
    dropped["deliveryDay"] = {"date": None, "timeWindowSettings": {}}
