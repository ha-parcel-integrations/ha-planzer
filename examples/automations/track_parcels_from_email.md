# Automatic parcel tracking from e-mail (IMAP → Planzer)

Companion guide for [`track_parcels_from_email.yaml`](track_parcels_from_email.yaml): watch your mailbox(es) for shipping e-mails, extract the Planzer tracking code, and register it with `planzer.track_parcel` — fully automatic, no extra custom component required.

Planzer is a **code-based** carrier: it has no account inbox, so every parcel must be registered by its tracking code before the integration can follow it. This recipe automates exactly that step.

**How it works, in one line:** the core [IMAP integration](https://www.home-assistant.io/integrations/imap/) fires an `imap_content` event for every new e-mail (including the body); the automation extracts the tracking code — a cheap regex first, an optional AI fallback for everything else — and calls `planzer.track_parcel`.

```
new e-mail ──imap_content──▶ automation ──▶ regex match? ──▶ planzer.track_parcel
                                     │
...
- This integration, with the `planzer.track_parcel` action available (field `tracking_code`).
...
A Planzer shipment number is a **bare integer** (`12345678`). That is hopeless as an e-mail regex — it would match prices, dates, invoice lines and half of every order confirmation. So the regex anchors on the **IKEA Switzerland order-number** form instead, which is distinctive:

```
(?<![0-9.])[0-9]{4,6}[.][0-9]{8,12}(?![0-9.])
```

That matches `98765.0012345678` and hands it to `planzer.track_parcel` **whole** — the integration derives the shipment number (`12345678`) itself, so the automation never has to do the string surgery.

**If your mails carry the bare shipment number instead**, don't widen this regex; let the AI fallback below pick it out, or add the parcel by hand. A pattern loose enough to catch a bare 8-digit number will track your invoice total.
...
1. **Jinja eats backslashes in string literals.** A template stored as `regex_findall('\b[0-9]…')` silently becomes a **backspace character** (`\b` is a string escape), so the regex never matches — no error anywhere. That's why the pattern is backslash-free: lookarounds instead of `\b`, `[0-9]` instead of `\d`, and `[.]` instead of `\.` for the dot in the IKEA order number. Copy that style if you change it.
...
curl -X POST -H "Authorization: Bearer $HA_TOKEN" -H "Content-Type: application/json" \
  ...
  -d '{"sender":"noreply@planzer.ch","subject":"Your parcel is on its way",
       "text":"Ihre Bestellung 98765.0012345678 ist unterwegs","initial":true,"folder":"INBOX","username":"test"}'
```

Then `planzer.untrack_parcel` the test code afterwards. For a full end-to-end test, forward a real shipping mail to the watched mailbox — it must arrive **unread**.
